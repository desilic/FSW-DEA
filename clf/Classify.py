...
def ensemble_predict_by_voted(cur_dim_clf, x_test):
    y_preds = [clf.predict(x_test) for clf in cur_dim_clf]
    y_preds = np.array(y_preds).reshape(len(y_preds), -1)#转为1行
    y_pred = np.mean(y_preds, axis=0) > 0.5
    return y_pred

def ensemble_predict_proba(cur_dim_clf, x_test):
    """
    soft voting: 求每个base的prob的均值即可
    :param cur_dim_clf:
    :param x_test:
    :return:
    """
    y_pred_probas=[clf.predict_proba(x_test) for clf in cur_dim_clf]
    y_pred_proba=np.zeros(y_pred_probas[0].shape)
    for i in y_pred_probas:
        y_pred_proba+=np.array(i)
    y_pred_proba=y_pred_proba/len(y_pred_probas)
    return y_pred_proba

class ClassifierMixin:
    def score(self, X, y, sample_weight=None):#手写了1个score
...
class Classifier(ClassifierMixin):
    ...
    def _build_ensemble(self):#实例化base->构建集成分类器
        '''
        ...
        MNB = GaussianNB()
        self.ensembles = [LDA, KNN, DTC, svc, LR, MLP, MNB]
        return self.ensembles

    def fit(self, x_train, y_train):#训练集成分类器
        '''
    ...
    def find_best_ensembles(self, x_test, y_test, accuracy=False):#分别训练，返回最好的分类器（有两种评价标准）
        '''
        ...
        # argsort()函数是将x中的元素从小到大排列，提取其对应的原index(索引)-----np.argsort()[-1]即输出x中最大值对应的index
        idx = np.argsort(scores)[-1]
        self.best_clf =self.ensembles[idx]
        self.best_score = scores[idx]
        return idx, self.best_clf, self.best_score

    def _predict_by_voted(self, x_test):#法一：投票决定分类结果
        '''
            仅适用二分类情况(0,1)
            投票过半数计为1, 否则计为0
        '''
    ...
    def _predict_by_best_clf(self, x_test):#法二：用最好的分类器预测
        '''
        直接调用分类器自带的predict方法
        '''
    ...
    def predict(self, x_test, vote=False):#开始预测
        '''
        两种预测方法
    ...
    def ErrOOB_score(self, y_true, y_pred):#用混淆矩阵衡量分类准确率
        '''
        对于二分类
        TP:真阳--原本是正例，预测为正例
        TF:真阴--原本是反例，预测为反例
    ...
    def get_mixture_matrix(self, y_true, y_pred):#手动实现混淆矩阵，返回4个指标
        '''
        对于二分类
        TP:真阳--原本是正例，预测为正例
        TF:真阴--原本是反例，预测为反例
...
def Error_score(y_true, y_pred):
    ...
    err_metrics={"TP":TP,
             ...
             "F1_measure":round(F1_measure, 3)
             }
    return 1 - (FN/(TP+FN) + FP/(TN+FP))/2, err_metrics
