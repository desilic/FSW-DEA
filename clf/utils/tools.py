...
from clf.Classify import Classifier
...
def load_feature_impts_from_dir(file_dir= r'save_vimps', show=False):
    '''
        导入基因重要性特征
    '''
    #注意：这些file就是来自feature_dic-即（显著度:抽中次数）
    ...
    for i, file in enumerate(file_list[:]):
        ...
        if i == 0:
            ...
            if np.sum(tmp[:,0] != 0) == 0:#对显著度不为0的行sum,ruo
                print(file)
                pass
            else:
                data += tmp#对基因的在所有抽样中的显著度求和
            # data += np.load(file_path)

    vimps = data[:,0] / data[:,1] # 总显著度/抽中次数-（相当于求平均-假设每个基因都只抽到1次的显著度）
    if show == True:
        fig, ax = plt.subplots()
        ax.plot(np.arange(data.shape[0]), data[:,1], '.k')
        ax.set_xlabel('Feature Index')
        ax.set_ylabel('Checked feature Count')
        ax.set_title('Feature Count')
        #保存图片为jpg，也可以pdf
        # plt.savefig(r'../static/img/feature_count.jpg',
        #             dpi=400, bbox_inches='tight')
        fig.savefig(r'static/img/feature_count.jpg', dpi=400, bbox_inches='tight')
        #plt.show()#统计每个特征的出现次数的图呗


        fig, ax = plt.subplots()
        ax.plot(np.arange(data.shape[0]), vimps, '.k')
        sorted_index = np.argsort(-vimps)
        for i in range(0):
            ax.text(sorted_index[i], vimps[sorted_index[i]], str(sorted_index[i]))
        ax.set_title('Gene Importance')
        ax.set_xlabel('Feature Index')
        ax.set_ylabel('Feature Score')
        # plt.ylim([0,0.25])
        #保存图片
        # plt.savefig(r'../static/img/gene_importance.jpg',
        #             dpi=400, bbox_inches='tight')#虽然tools在clf下的utils里，但系统默认当前路径时clf，因为运行的code时test.py(在clf下)，tools.py只是调用的模块
        fig.savefig(r'static/img/gene_importance.jpg', dpi=400, bbox_inches='tight')#因为把tools的code放到了serve里，所以. 变为项目根路径了
        #plt.show()
        #print(sorted_index[:20])
    return vimps#每个gene的平均显著度
...
class GraphVimps:
    ...
    def update(self, n_feature_vimps, best_score):
        ...
        # if best_score != 0:
        keys, values = np.array(list(n_feature_vimps.keys())), np.array(list(n_feature_vimps.values()))#keys:特征编号，values:显著程度
...
def split_gaussian(Weight1, Weight2, Miu1, Miu2, delta1, delta2):# Miu:均值，delta:标准差
    """
        通过高斯密度函数计算两个高斯之间的交点
    """
    #这个参数weight是啥意思？？？---混合高斯分布，每个高斯的权重（总权重和=1）
...
def showfigure(weights,means,covars, data_vector,gaussian_boundary):
    ...
    for x in gaussian_boundary:
        ax.plot([x,x],[0,13], 'b')  #添加1条竖线
    ax.plot(data_vector, np.zeros(data_vector.shape),'.k', markersize=8)
    ax.set_ylabel('Density')
    ax.set_ylim((-15, 400))
    ax.set_xlim((-0.03,0.3)) #在实际取不到负值下，设置不太大的负刻度，相当于把坐标轴向上/右平移1些，可能会更好看
    # plt.xlabel('Feature Index')
    ax.set_xlabel('Accumulated importance')
    ax.set_title("Gauss")
    # plt.tick_params(labelsize = 18)

    # plt.savefig(r'../static/img/Gauss.jpg',
    #             dpi=400, bbox_inches='tight')
    fig.savefig(r'static/img/Gauss.jpg',
                dpi=400, bbox_inches='tight')
    #plt.show()
...
def get_gaussian_boundary(data, n_components=5, show=False):
    """" 按照1424个特征的显著程度，将特征分为1->5类，所以就是把前i类的所有特征当作重要特征（基因）"""
    #data:基因重要性，n_components:指定聚类中心的个数5（使用5个高斯分布进行拟合）,这里用的是一维混合高斯模型（因为每个特征都是一维的）
    X = data.reshape(-1,1)#转二维array
    lowest_bic = np.infty#设为无穷
    bic = []#评估高斯混合GMM（高斯聚类）的指标---越低越好
    #枚举将特征聚为几类
    ...
    #    gmm = mixture.GaussianMixture(n_components=n_components, covariance_type='spherical').fit(X)
        ...
        bic.append(gmm.bic(X))#找bic值最小的model为best_model，也就是说BIC可以作为高斯聚类模型的选择参数
        if bic[-1] < lowest_bic:#打擂法找bic最小
            lowest_bic = bic[-1]
            best_gmm = gmm

    #解GMM模型，实际上就是确定GMM模型的参数(μ,Σ,π) (mu ,Sigma ,pi ):（均值，协方差，权重）
    means = best_gmm.means_.flatten()#5类（5个高斯）各自的？均值
    covars = best_gmm.covariances_.flatten()#5类的各自协方差，还是方差？（因为是一维高斯，所以就是方差，方差是特殊的协方差）
    ...
    index = np.argsort(means)#按均值排序（因为高斯按均值miu确定位置）
    for i in range(len(index)-1):#找每相邻两个高斯的分界
